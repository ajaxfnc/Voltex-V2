...
const registerCommands = (client) => {
  ...
  for (const type of readdirSync("/root/ghoul trooper testbot/src/commands/")) {
    for (const dir of readdirSync("/root/ghoul trooper testbot/src/commands/" + type)) {
      for (const file of readdirSync("/root/ghoul trooper testbot/src/commands/" + type + '/' + dir).filter((f) => f.endsWith('.js'))) {
